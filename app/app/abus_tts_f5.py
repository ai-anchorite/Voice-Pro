...
class F5TTS:
    ...
    def generate_audio(self, dubbing_text:str, output_file, ref_audio, ref_text, speed_factor, progress=gr.Progress()):
        ...
        try:
            ...
            logger.warning(f"[abus_tts_f5.py] infer_process - issue: {e}")        
    ...
    def request_tts(self, line: str, output_file: str, ref_audio, ref_text, speed_factor, audio_format):
        # Use .wav for intermediate since soundfile can't encode mp3
        output_voice_file = os.path.join(path_dubbing_folder(), path_new_filename(ext=".wav"))
        line = AbusText.normalize_text(line)
        if len(line) < 1:
            logger.warning(f"[abus_tts_f5.py] request_tts - warning: no line")
            return False
        ...
        if not os.path.exists(output_voice_file) or os.path.getsize(output_voice_file) < 100:
            logger.warning(f"[abus_tts_f5.py] request_tts - TTS produced empty/missing file")
            return False
        ...
        try:
            ...
            logger.warning(f"[abus_tts_f5.py] request_tts - trim/export issue: {e}")
            try:
                raw_audio = AudioSegment.from_file(output_voice_file)
                raw_audio.set_channels(2).export(output_file, format=audio_format)
            except Exception as e2:
                logger.warning(f"[abus_tts_f5.py] request_tts - fallback also failed: {e2}")
                return False
        finally:
            try:
    ...
    def infer_multi(self, dubbing_text:str, output_file, celeb_audio1, celeb_transcript1, celeb_audio2, celeb_transcript2, model_choice, speed_factor, audio_format: str, progress=gr.Progress()):
        ...
        try:
            ...
            logger.warning(f"[abus_tts_f5.py] infer_multi - An issue occurred: {e}")
        finally:
            del self.ema_model
            self.ema_model = None
            self.release_cuda_memory()
