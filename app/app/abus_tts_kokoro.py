...
class KokoroTTS:
    ...
    def request_tts(self, line: str, output_file: str, kokoro_voice, speed_factor, audio_format):
        # Use .wav for the intermediate file since soundfile can't encode mp3
        output_voice_file = os.path.join(path_dubbing_folder(), path_new_filename(ext=".wav"))
        line = AbusText.normalize_text(line)
        if len(line) < 1:
            logger.warning(f"[abus_tts_kokoro.py] request_tts - warning: no line")
            return False
        
        try:
            pipeline = KPipeline(lang_code=kokoro_voice.lang_code)
        except Exception as e:
            logger.warning(f"[abus_tts_kokoro.py] request_tts - Failed to initialize KPipeline: {e}")
            return False
        ...
        if not os.path.exists(output_voice_file) or os.path.getsize(output_voice_file) < 100:
            logger.warning(f"[abus_tts_kokoro.py] request_tts - TTS produced empty/missing file")
            return False
        
        try:
            # Use pydub for the entire trim + convert + stereo pipeline
            trimmed = AbusAudio.trim_silence_audio(output_voice_file)
            # Export directly to the final output file with the correct format
            trimmed.set_channels(2).export(output_file, format=audio_format)
        except Exception as e:
            logger.warning(f"[abus_tts_kokoro.py] request_tts - trim/export issue: {e}")
            ...
            try:
                ...
                logger.warning(f"[abus_tts_kokoro.py] request_tts - fallback also failed: {e2}")
                return False
        finally:
            try:
