...
class EdgeTTS:
    ...
    def request_tts(self, line: str, output_file: str, voice_name: str, semitones, speed_factor, volume_factor, audio_format):
        # edge-tts always outputs mp3, so use .mp3 for the intermediate file
        output_voice_file = os.path.join(path_dubbing_folder(), path_new_filename(ext=".mp3"))
        line = AbusText.normalize_text(line)
        if len(line) < 1:
            logger.warning(f"[abus_tts_edge.py] request_tts - warning: no line")
            return False
        ...
        if not os.path.exists(output_voice_file) or os.path.getsize(output_voice_file) < 100:
            logger.warning(f"[abus_tts_edge.py] request_tts - TTS produced empty/missing file")
            return False
        ...
        try:
            ...
            logger.warning(f"[abus_tts_edge.py] request_tts - trim/export issue: {e}")
            try:
                raw_audio = AudioSegment.from_file(output_voice_file)
                raw_audio.set_channels(2).export(output_file, format=audio_format)
            except Exception as e2:
                logger.warning(f"[abus_tts_edge.py] request_tts - fallback also failed: {e2}")
                return False
        finally:
            try:
