...
def sanitize_path_component(name):
    """Strip emojis and characters that break os.system/cmd.exe on Windows.
    Keeps accented Latin characters (é, ñ, ü, etc.) but removes emojis and
    other characters outside the Windows-1252 codepage."""
    result = []
    for ch in name:
        try:
            ch.encode('cp1252')
...
def path_add_postfix(file_path: str, postfix: str, ext: str = ""):
    file_name, file_extension = os.path.splitext(os.path.basename(file_path))
    # Sanitize postfix to remove emojis/non-ASCII that break os.system/cmd.exe on Windows
    postfix = sanitize_path_component(postfix) if postfix else postfix
    if ext == "":
