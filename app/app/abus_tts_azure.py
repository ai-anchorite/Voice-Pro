...
class AzureTTS:
    ...
    def request_tts(self, line: str, output_file: str, voice_name: str, semitones, speed_factor, volume_factor, audio_format):
        ...
        if False == self.generate_audio(line, voice_name, output_voice_file, rate=speed_factor, volume=volume_factor, pitch=semitones):
            logger.warning(f"[abus_tts_azure.py] request_tts - warning: API returns False")
            return False
        ...
        try:
            ...
            logger.warning(f"[abus_tts_azure.py] request_tts - trim/export issue: {e}")
            try:
                raw_audio = AudioSegment.from_file(output_voice_file)
                raw_audio.set_channels(2).export(output_file, format=audio_format)
            except Exception as e2:
                logger.warning(f"[abus_tts_azure.py] request_tts - fallback also failed: {e2}")
                return False
        finally:
            try:
